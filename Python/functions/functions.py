...
# Подробнее про это в файле func_programming_functions.py

# 1. sum - находит сумму всех элементов iterable.
# 2, 3. min, max - находит минимум и максимум в последовательности iterable.
# 4. sorted - возвращает отсортированный iterable (подробнее в файле lists.py)
# 5. map() - применяет некоторую функцию к каждому элементу iterable.
# Возвращает объект iterable, где каждый элемент объекта был подвергнут функции-предикату
print(list(map(lambda x: x * x, [1, 2, 3, 4, 5, 6, 7, 8, 9])))  # => [1, 4, 9, 16, 25, 36, 49, 64, 81]

# 6. filter() - фильтрация данных по какому-то параметру.
...
print(*filter(lambda x: x > 0, [1, 2, -1, 5, 6, 0, -2, 2]))  # => 1 2 5 6 2
