...
# Можно передать в качестве параметра строку, которая будет выводиться на экран перез запросом input:
name = input('Введите свое имя: ')  # Выведет 'Введите свое имя: ' и будет ждать ввода пользователя
...
# О логических операция в data_types/bool.py
