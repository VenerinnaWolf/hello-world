...
<!--            Далее идет блок с информацией о --- формах ---            -->
...
<!--            ----------------------------            -->
    ...
    <!-- Перепишем форму выше под метод POST: -->
...
<!--            -----------------------------            -->
    ...
    <form>
        <input type="text" name="product[name]">    <!-- Окошко формы 1 -->
        <input type="text" name="product[article]"> <!-- Окошко формы 2 -->
        <input type="text" name="product[price]">   <!-- Окошко формы 3 -->
        <input type="submit">   <!-- Кнопка "Отправить" -->
    </form>

    <!-- При отправке формы выше, содержимое $_GET будет таким:
        Array
        (
            [product] => Array
                (
                    [name] => То, что ввели в окошко 1
                    [article] => Окошко 2
                    [price] => Окошко 3
                )
        )   
    ...
    <!-- Пример обработчика формы выше: (важно! обработчик нужно писать ДО самой формы, просто я не хочу копировать ее еще раз снизу) -->
    <?php
    ...
    <!-- Ключами могут быть и обычные числа -->
    ...
    <!-- Пример обработчика для такой формы -->
    <?php
...
<!--            ---------------------            -->
    ...
    <!-- Полная статья со всеми объяснениями кода - https://www.programulin.ru/dinamicheskie-stranicy-v-php -->

    <!-- Динамическая страница - это PHP скрипт, который в зависимости от URL отображает разный контент. -->

    <!-- Динамический массив, в котором лежат все страницы. Далее мы будем использовать их номера в качестве id-->
    <?php
        $articles = [ 
            [                                                   // Первая статья (id = 0)
    ...
    <!-- Выводим меню (для каждой статьи мы выводим ее название с кликабельной ссылкой) -->
    <a href="/">Главная</a>         <!-- По ссылке .../ будет лежать главная страница (... - это наш домен, у меня https://hello-world) -->
    <br>
    <?php foreach($articles as $id => $article): ?>
        <a href="/index.php?id=<?= $id ?>"><?= $article['title'] ?></a>     <!-- По ссылке .../index.php?id=0 или id=1 и тд будут лежать остальные страницы 
                                                                            (Вместо index.php пишем скрипт, в котором запускается код, если он другой) -->
        <br>
    <?php endforeach; ?>        <!-- Конструкция для завершения цикла, как endif -->

    <!-- Теперь отобразим контент каждой страницы при нажатии на ссылки -->
    <?php
        // При создании ссылок мы уже указывали id=0 или id=1, поэтому в массиве _GET для каждой ссылки есть элемент под ключом id: $_GET['id'] с определенным значением: 0 или 1
...
<!--            Далее идет блок с информацией о --- файлах ---            -->
...
<?php
    ...
    // ---------------
        ...
        // Допустим, в корне сайта лежат файлы index.php и config.php. Мы можем в первый файл подключить второй (его код выполнится в месте, где использовалась команда include):

        // Файл index.php
            include 'config.php';   // Указываем относительный путь к скрипту
            echo 'Это главный файл.';

        // Файл config.php
            echo 'Это файл config.';
        
        // При запуске скрипта index.php мы получим результат: Это файл config.Это главный файл.
    ...
    // ---------------
        ...
        /*
            ...
            Разница в том, что при невозможности подключения файла (файл отсутствует или недостаточно прав) функция include() покажет ошибку и продолжит работу, 
            а require() покажет ошибку и остановит выполнение скрипта.        
        */ 
    ...
    // -----------------------------------
        ...
        /*
            При использовании функций с приставкой _once вы запрещаете повторное подключение скрипта. 
            При попытке повторного подключения PHP сгенерирует ошибку.
    ...
    // -----------------------------------------
        ...
        // Файл index.php
            $config = include('config.php');    // Создаем переменную, которая примет массив из файла config.php
            echo $config['site_name'];

        // Файл config.php
            return [    'site_url' => 'www.programulin.ru',   // Возвращаем массив
    ...
    // -------------------------------------
        ...
        /*
            Все переменные, функции и т.п., объявленные в подключаемом файле, будут видны в исходном. 
            При этом переменные подключаются в текущую область видимости. 
            Т.е. если подключить файл внутри функции, то переменные будут видны только в ней:
        */ 
...
<?php
    ...
    // ---------------------
        // Абсолютный путь - это полный путь к папке или файлу.

        // Абсолютный путь скрипта можно получить с помощью константы __FILE__ (подчеркивания тут двойные):
        echo __FILE__;

        // Абсолютный путь к папке, в которой лежит скрипт, можно получить с помощью константы __DIR__:
        echo __DIR__;

        // К примеру, чтобы подключить config.php, который лежит в той же папке, что текущий скрипт, можно:
        require 'C:\OSPanel\home\hello-world\public\config.php';     // Указать полный путь вручную
        require __DIR__ . '/config.php';    // Подставить путь к текущей папке и добавить имя файла вручную (обязательно добавить слеш, причем не такой, как в файловой системе Windows)
    ...
    // ------------------------
        ...
        /*
            Алгоритм поиска скрипта компьютером по относительному пути в PHP
            1) PHP попытается найти этот файл в папках, указанных в директиве include_path. 
            Посмотреть, что указано в этой директиве конкретно у вас можно с помощью var_dump(get_include_path());, папки разделяются символом ;
            Если мы укажем путь к скрипту в таком виде: require('./config.php');, то этот шаг будет пропущен.
            2) Затем PHP попытается найти файл в папке, в которой находится подключающий скрипт (в нашем случае index.php).
            3) Далее PHP попытается найти файл в папке текущего рабочего каталога.
            ...
            Поэтому есть шанс, что подключится не нужный нам файл. Так что лучше использовать абсолютный путь
        */ 
    ...
    // ------------------------
        ...
        // Относительные пути в URL указываются без слеша в начале ссылки, например:    <a href="articles">Ссылка</a>
        
        /*
            Примеры ошибок в использовании относительных путей:

            1) Ошибка №1: относительные пути к стилям, скриптам и другим файлам
                Допустим, подключим стили к нашему сайту:
                <link rel="stylesheet" type="text/css" href="style.css">
                На главной странице все работает, но при переходе по любой ссылке (например, /products/15),
                уже нет, т.к. относительный путь строится от текущего URL-адреса, а значит в нашем примере он из style.css превратился в /products/15/style.css

            2) Ошибка №2: Рекурсия в ссылках
                <a href="articles/about">О нас</a>
                При каждом клике по этой ссылке, текущий URL будет не перезаписываться, а дополняться, т.е. получится что-то вроде http://test.ru/articles/articles/articles/articles/about
        */ 
    ...
    // В любых ссылках мы можем указать "перейти на папку выше", например:
    include '../config.php';    // Относительный путь к родительской папке
    include __DIR__ . '/../config.php';     // Абсолютный путь
    // <a href="../article.php">Статья</a>      - URL-адрес
    include './config.php';     // Ссылка на текущий каталог (иногда может быть нужно)
?>