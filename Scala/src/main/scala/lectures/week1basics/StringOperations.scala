...
@main def StringOperations(): Unit = {
  ...
  // .charAt(index) - возвращает символ строки, находящийся на указанной позиции (index) в строке.
  //                  В качестве индекса допускается указывать значения от 0 до length - 1
  ...
  // .take(n) - возвращает строку, состоящую из n первых символов исходной строки;
  //            метод take направлен на взятие первых n элементов, n не может быть отрицательным
